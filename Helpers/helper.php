...
class Helper
{
    public function generatePagination($limit, $page, $count)
    {
    ...
    public function generateAdminPagination($limit, $page, $count) {
        $queryString = $_SERVER["QUERY_STRING"];
        parse_str($queryString, $queryArray);
        $maxRange = 3;
        $numberOfPage = ceil($count / $limit);
        ...
        if ($page > 1) {
            $queryArray["page"] = $page - 1;
            $queryStringConverted = http_build_query($queryArray);
            $pagination .= "<li> <a href='?$queryStringConverted' aria-label='Previous'><span class='fa fa-angle-double-left' aria-hidden='true'></span></a></li>";
        }
        ...
        if ($page < $numberOfPage) {
            $queryArray["page"] = $page + 1;
            $queryStringConverted = http_build_query($queryArray);
            $pagination .= "<li><a href='?$queryStringConverted' aria-label='Next'><span class='fa fa-angle-double-right' aria-hidden='true'></span></a></li>";
        }
